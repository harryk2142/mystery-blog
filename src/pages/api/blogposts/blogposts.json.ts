export function GET({ params, request }) {
	return new Response(JSON.stringify(blogposts), {
		status: 200,
		headers: {
			"Content-Type": "application/json",
		},
	});
}

const blogposts = [
	{
		title: "Schneeglöckchen und Schattenwesen",
		headline:
			"Schneeglöckchen und Schattenwesen – Wenn der Frühling etwas Düsteres weckt 🌸",
		img: "/images/blog/geheimnisvoller-fruehlingswald-schneegloeckchen-daemmerung.webp",
		alt: "Geheimnisvoller Frühlingswald bei Morgendämmerung mit goldenem Licht, blühenden Schneeglöckchen und einer schemenhaften Gestalt im Hintergrund.",
		pubDate: "2025-03-22T13:28:38.293Z",
		url: "schneegloeckchen-und-schattenwesen",
	},
	{
		title: "Das Februar Biest",
		headline:
			"Das Februar-Biest: Wenn der Winter nicht das Einzige ist, das dich jagt",
		img: "/images/blog/dunkler-winterwald-geheimnisvolle-fussspuren.webp",
		alt: "„Düstere, verschneite Waldlandschaft in der Nacht mit einer verlassenen Hütte im Hintergrund. Riesige, krallenartige Fußspuren im Schnee führen ins Dunkel, aus dem rote Augen bedrohlich leuchten.“",
		pubDate: "2025-02-23T16:47:31.000Z",
		url: "das-februar-biest",
	},
	{
		title: "Bigfoot in Deutschland gesichtet - Mit Foto - Mysteriöse Begegnung in Dunkelheim",
		headline:
			"Bigfoot in Deutschland: Mysteriöse Begegnung in der malerischer Altstadt von Dunkelheim",
		img: "/images/blog/bigfoot-von-hinten-in-einer-gasse-einer-deutschen-altstadt.webp",
		alt: "Bigfoot von Hinten in einer Gasse einer deutschen Altstadt",
		pubDate: "2025-01-15T13:35:30.000Z",
		url: "bigfoot-in-deutschland-gesichtet",
	},
	{
		title: "Das Ritual der Zwölf: Das unheimliche Silvestermysterium von Frosthain",
		headline: "Das Ritual der Zwölf - Wer sind die Fremden in der Neujahrsnacht?",
		img: "/images/blog/verschneites-mystisches-dorf-mit-verhuellten-gestalten-bei-nacht.webp",
		alt: "Ein verschneiter Dorfplatz bei Nacht, umgeben von alten Fachwerkhäusern. Zwölf dunkle, verhüllte Gestalten stehen auf dem Platz, während der Mond durch Wolken ein schwaches Licht auf die Szene wirft. Im Hintergrund führt ein beleuchteter Weg zu einer verlassenen Kapelle am Waldrand, hinter der Nebelschwaden aus einem dunklen Wald aufsteigen.",
		pubDate: "2024-12-31T11:45:07.000Z",
		url: "das-ritual-der-zwoelf-das-unheimliche-silvestermysterium-von-frosthain",
	},
	{
		title: "Das Geheimnis von Altweidlingen: Die mysteriöse verschwundene Zeit",
		headline:
			"Das Novembergeheimnis der verschwundenen Zeit: Wer oder was steckt dahinter?",
		img: "/images/blog/novembergeheimnis-altweidlingen-verschwundene-zeit.webp",
		alt: "Nebelverhangenes Dorf mit altem Kirchturm und stehengebliebener Uhr auf einem leeren Platz, umgeben von Herbstlaub und schummrigem Licht.",
		pubDate: "2024-11-10T11:46:33.732Z",
		url: "novembergeheimnis-verschwundene-zeit-altweidlingen",
	},
	{
		title: "Nebelfiguren im Teufelsmoor",
		headline: "Nebelfiguren über dem Moor: Wer wandelt in der Dämmerung?",
		img: "/images/blog/nebliges-geheimnisvolles-moor.webp",
		alt: "Ein nebliges Moor bei Dämmerung mit kargen Bäumen und geheimnisvollen Schattenfiguren, die im dichten Nebel auftauchen. Die unheimliche Atmosphäre wird durch das schwache Licht und die langen Schatten verstärkt.",
		pubDate: "2024-10-12T15:30:22.538Z",
		url: "nebelfiguren-im-teufelsmoor",
	},
	{
		title: "Exklusive Einblicke zum Sternentor aus Sektor 7",
		headline:
			"Geheimlabor Sektor 7: Exklusive Einblicke in das Labor, dass das Sternentor aktiviert hat",
		img: "/images/blog/ein-hochdetailiertes-und-realistisches-bild-eines-stargates-das-sich-in-einem-high-tech-labor-befindet.webp",
		alt: "Großes Sternentor inmitten eines Raums mit vielen Wissenschaftlern",
		pubDate: "2024-09-06T18:24:53.633Z",
		url: "exklusive-einblicke-zum-sternentor-aus-sektor-7",
	},
	{
		title: "Unheimliche Botschaft im Kornfeld",
		headline: "Das Geheimnis der rätselhaften Botschaft im Kornfeld von Gruselhain",
		img: "/images/blog/kornfeld-mit-der-nachricht-das-ende-ist-nah.webp",
		alt: 'Ein geheimnisvolles Kornfeld bei Sonnenaufgang mit der Botschaft "Das Ende ist nah!" im Nebel, ungewöhnliche Lichter am Himmel und ein Dorf im Hintergrund.',
		pubDate: "2024-08-07T19:06:22.224Z",
		url: "unheimliche-botschaft-im-kornfeld",
	},
	{
		title: "Das verschwundene Dorf: Rätsel um Mügelfurt",
		headline: "Das Mysterium von Mügelfurt: Ein Dorf verschwindet spurlos",
		img: "/images/blog/mysterioeses-verlassenes-dorf-in-einem-dichten-wald.webp",
		alt: "Ein mysteriöses, verlassenes Dorf in einem dichten Wald. Traditionelle deutsche Häuser stehen mit leicht geöffneten Türen, und dichte Nebelschwaden bedecken das Gebiet, was ihm eine geisterhafte Atmosphäre verleiht. Im Hintergrund erhebt sich ein dunkler, unheimlicher Wald mit Schatten, die an eine geisterhafte Gestalt erinnern. Die gesamte Szenerie wirkt dunkel und geheimnisvoll, passend zu einem übernatürlichen Ereignis.",
		pubDate: "2024-07-23T19:10:55.534Z",
		url: "dorf-in-deutschland-verschwunden",
	},
	{
		title: "Ein Alien in einem Glastank der 60er Jahre entdeckt",
		headline:
			"Das Geheimnis des außerirdischen Wesens im Glastank: Ein Fund aus den sechziger Jahren",
		img: "/images/blog/foto-wissenschaftler-stehen-neben-alien.webp",
		alt: "Ein Foto mit einer Gruppe von drei Wissenschaftlern in weißen Laborkitteln, die um einen großen Glastank stehen. Im Tank befindet sich ein außerirdisch aussehendes Wesen mit humanoiden Zügen. Das Wesen hat einen großen Kopf, dünne Gliedmaßen und eine schlanke, strukturierte Körperform.",
		pubDate: "2024-06-25T19:21:26.641Z",
		url: "foto-eines-aliens-in-einem-glastank",
	},
	{
		title: "Die Nacht der verschwundenen Maibäume: Ein Mysterium in Marburg",
		headline:
			"Die Nacht der verschwundenen Maibäume: Ein Mysterium in Marburg - Otto Normalverbraucher untersucht",
		img: "/images/blog/mysterioes-leuchtender-baum-nachts-in-einer-traditionellen-deutschen-stadt.webp",
		alt: "Das Bild zeigt eine nächtliche Szene in Marburg, Deutschland. Im Zentrum steht ein majestätischer Maibaum, der in einem traditionellen deutschen Stadtplatz wieder auftaucht. Der Platz ist von alten Fachwerkhäusern umgeben, die im sanften Mondlicht erstrahlen. Mystische Schatten und geisterhafte Figuren sind im Nebel zu erkennen, während blaue und grüne Lichter über dem Boden schweben und eine unheimliche Atmosphäre schaffen. Die Silhouetten staunender Menschen sind um den Maibaum herum zu sehen, fasziniert von der Rückkehr der Bäume. Die gesamte Szene ist in einem weiten Format gehalten, das die Weite und Magie des Augenblicks einfängt.",
		pubDate: "2024-05-24T22:35:10.217Z",
		url: "die-nacht-der-verschwundenen-maibaeume-ein-mysterium-in-marburg",
	},
	{
		title: "Microsoft und OpenAI sagen die Zukunft voraus",
		headline: "Die neue Ära der KI: Microsoft und OpenAI's mysteriöse Enthüllung",
		img: "/images/blog/microsoft-openai-project-cassandra-presentation.webp",
		alt: "Das Bild zeigt eine hochmoderne Präsentation im Microsoft Campus in Redmond, Washington, bei der Satya Nadella und Sam Altman das Projekt Cassandra vorstellen. Auf der Leinwand sind die Logos von Microsoft und OpenAI zu sehen, während die Zuschauer gespannt den futuristischen holografischen Displays folgen.",
		pubDate: "2024-05-19T18:44:29.525Z",
		url: "microsoft-und-openai-sagen-die-zukunft-voraus",
	},
	{
		title: "Der Spuk im Spargelfeld",
		headline: "Der Spuk im Spargelfeld: Ein phantastisches Mai-Rätsel in Beelitz",
		img: "/images/blog/nachtszene-auf-einem-spargelfeld-mit-geisterhaften-erscheinungen-und-unerklaerlichen-lichtern.webp",
		alt: "Hier ist das Bild, das zu deinem Artikel über das mysteriöse Geschehen in den Spargelfeldern von Beelitz passt. Es zeigt die nächtliche Szene mit den geisterhaften Erscheinungen und den unerklärlichen Lichtern, genau wie beschrieben.",
		pubDate: "2024-05-13T15:09:43.990Z",
		url: "spuk-im-spargelfeld",
	},
	{
		title: "Der geheimnisvolle erste Mai in Deutschland",
		headline:
			"Der Geheimnisvolle Erste Mai: Traditionen, Mythen und Mysterien in Deutschland",
		img: "/images/blog/festliche-szene-im-deutschen-dorf-am-ersten-mai.webp",
		alt: "Festliche Szene in einem deutschen Dorf am ersten Mai. In der Mitte steht ein Maibaum",
		pubDate: "2024-05-01T11:45:59.493Z",
		url: "der-geheimnisvolle-erste-mai",
	},
	{
		title: "Geheimnisvolles Artefakt in der Schweiz entdeckt",
		headline: "Mitten in Europa - Geheimnisvolles Artefakt in der Schweiz entdeckt",
		img: "/images/blog/metallisches-objekt-auf-dem-boden-im-hintergrund-ein-dorf-zu-sehen.webp",
		alt: "Schöne Landschaft. Rundes, metallisches Objekt auf dem Boden. Im Hintergrund ist ein Dorf zu sehen.",
		pubDate: "2024-04-10T18:46:33.730Z",
		url: "geheimnisvolles-artefakt-in-der-schweiz-entdeckt",
	},
	{
		title: "Das Lichtphänomen von Hessdalen",
		headline: "Ufo Hunters nächtliche Jagd auf Hessdalens geheimnisvolle Lichter",
		img: "/images/blog/hessdalen-lichter-beobachtung-nacht.webp",
		alt: "Ein Beobachter in Winterkleidung steht bei Nacht in einem verschneiten norwegischen Tal und dokumentiert mit Nachtsichtkamera und EMF-Messer ein spektakuläres, unerklärliches Lichtphänomen am Himmel, unterlegt von den Nordlichtern.",
		pubDate: "2024-03-20T19:51:35.503Z",
		url: "das-lichtphaenomen-von-hessdalen",
	},
	{
		title: "Rätselhafter Monolith in Wüstlingen entdeckt",
		headline:
			"Der Monolith von Wüstlingen - Ein Interview mit dem Entdecker Herrn Sandro Steinblick",
		img: "/images/blog/ein-reflektierender-monolith-steht-im-zentrum-einer-wuestenlandschaft.webp",
		alt: "Ein hoch aufragender, reflektierender Monolith steht im Zentrum einer weiten Wüstenlandschaft. Der Himmel zeigt ein Farbgradient von sanftem Rosa zu hellem Blau, was auf die frühe Morgendämmerung hinweist. Der Monolith spiegelt die Farben des Himmels auf all seinen Seiten wider und schafft eine surreale, geheimnisvolle Atmosphäre. Die Wüste ist karg mit rissigem Boden, und es gibt keine Fußabdrücke oder Anzeichen menschlicher Präsenz rund um den Monolithen. Die Szene vermittelt ein Gefühl der Isolation und des Wunders, als ob der Monolith aus dem Nichts erschienen wäre. Das Licht ist weich und die frühe Morgensonne wirft lange Schatten.",
		pubDate: "2024-02-03T10:43:47.822Z",
		url: "raetselhafter-monolith-in-wuestlingen-entdeckt",
	},
	{
		title: "Das Geheimnis des Schneefalls: Andy Türs Ungewöhnliche Wettertheorie",
		headline:
			"Unerklärlicher Schneefall in Deutschland: Meteorologe Andy Tür Enthüllt Seine Kuriose Theorie",
		img: "/images/blog/ein-mann-mit-einem-klemmbrett-steht-draussen-im-schnee.webp",
		alt: "Meteorologe Andy Tür beobachtet Schneefall in deutscher Winterlandschaft, mit spielenden Kindern und verschneiten Straßen",
		pubDate: "2024-01-19T17:48:24.601Z",
		url: "andy-tuer-geheimnis-schneefall",
	},
	{
		title: "Die Geisterzüge von Neujahr: Ein Rätsel im deutschen Schienennetz",
		headline: "Das Mysterium der Januar Geisterzüge",
		img: "/images/blog/geisterzug-in-deutscher-nachtlandschaft.webp",
		alt: "Ein altertümlicher, teilweise transparenter Geisterzug mit Dampf und unheimlichen Lichtern auf einem Eisenbahngleis in einer nebligen, sternenklaren Nacht in Deutschland, umgeben von einer dunklen, bewaldeten Landschaft.",
		pubDate: "2024-01-07T09:01:40.511Z",
		url: "das-mysterium-der-januar-geisterzuege",
	},
	{
		title: "Das weihnachtliche Wunder von Sphinxhausen: Die Rückkehr des Horus",
		headline: "Das weihnachtliche Wunder von Sphinxhausen: Die Rückkehr des Horus",
		img: "/images/blog/mystische-erscheinung-horus-sphinxhausen.webp",
		alt: "Magische nächtliche Szene in einem bayerischen Dorf mit der illuminierten Horus-Statue im Zentrum des Platzes, umgeben von staunenden Dorfbewohnern, vor dem Hintergrund traditioneller bayerischer Architektur unter einem Sternenhimmel.",
		pubDate: "2023-12-31T17:19:23.280Z",
		url: "der-fall-der-verschwundenen-horus-statue-in-sphinxhausen-teil-3",
	},
	{
		title: "Steht ein Wunder von Sphinxhausen bevor?",
		headline: "Das bevorstehende Wunder von Sphinxhausen - Die Rückkehr von Horus",
		img: "/images/blog/fröhliche-menschen-sphinxhausen-vorfreude-horus.webp",
		alt: "Fröhliche kleine Gruppe in traditioneller bayerischer und winterlicher Kleidung, die bei sanfter Beleuchtung in Sphinxhausen zusammensteht, lachend und warme Getränke haltend, in Erwartung der Rückkehr der Horus-Statue.",
		pubDate: "2023-12-24T23:21:58.514Z",
		url: "der-fall-der-verschwundenen-horus-statue-in-sphinxhausen-teil-2",
	},
	{
		title: "Rätsel um Horus-Statue in Sphinxhausen - Ein Mysterium",
		headline:
			"Der mysteriöse Fall der verschwundenen Horus-Statue in Sphinxhausen, Bayern",
		img: "/images/blog/menschen-in-wintersachen-auf-dem-marktplatz-im-verschneiten-bayerischen-dorf.webp",
		alt: "Eine Menschengruppe mit Winterklamotten steht auf dem Marktplatz in einem verschneiten bayerischen Dorf",
		pubDate: "2023-12-17T16:21:58.514Z",
		url: "der-fall-der-verschwundenen-horus-statue-in-sphinxhausen",
	},
	{
		title: "Die Legende des Winter-Einhorns in Frostfels",
		headline: "Ein Wunder im Winterwald: Das Weiße Einhorn",
		img: "/images/blog/weisses-einhorn-mit-schnee.webp",
		alt: "Ein weißes Einhorn mit einer schönen Winterlandschaft. Sonniger Tag.",
		pubDate: "2023-12-13T19:10:46.175Z",
		url: "das-winter-einhorn-ein-maerchenhaftes-geheimnis-im-schnee",
	},
	{
		title: "Das Geheimnis des Raunenwaldes in Nebelheim",
		headline:
			"Geheimnisse im Mondlicht: Die unerzählte Legende des Raunenwaldes in Nebelheim",
		img: "/images/blog/nachts-ein-mann-mit-einer-taschenlampe-im-dichten-wald.webp",
		alt: "Nächtlicher, nebelverhangener Wald mit beleuchtetem Pfad, umgeben von knorrigen Bäumen und einer schemenhaften Figur im Hintergrund.",
		pubDate: "2023-11-23T15:56:53.935Z",
		url: "das-geheimnis-des-raunenwaldes-in-nebelheim",
	},
	{
		title: "Mysteriöser Angriff in der Nähe von Tübingen",
		headline:
			"Unerklärlicher Angriff in Tübingen: Mysteriöse Flugmonster attackieren junge Leute - Was steckt dahinter?",
		img: "/images/blog/mysterioese-fliegende-kreatur.webp",
		alt: "Eine künstlerische Darstellung der Kreatur, basierend auf den Beschreibungen der Gruppe. Hintergrund: Eine nächtliche Szenerie mit einem Vollmond und Wolken, die den Himmel bedecken. Sterne funkeln durch die Wolkenlücken, und die Silhouette eines Waldes ist am Horizont zu sehen",
		pubDate: "2023-09-25T00:00:00.000Z",
		url: "fliegendes-monster-angriff-jugendliche-tuebingen",
	},
	{
		title: "Geister im Büro: Warum Ihr Drucker wirklich immer klemmt",
		headline: "Geister im Büro: Warum Ihr Drucker wirklich immer klemmt",
		img: "/images/blog/ein-geist-in-einem-office.webp",
		alt: "Ein mysteriöses Ding, das wie eine Mischung aus einem Alien, einem Roboter und einem Kristall aussieht. Schwebend über dem Boden.",
		pubDate: "2023-09-24T00:00:00.000Z",
		url: "geist-in-office-nebelstein-bayern",
	},
	{
		title: "Aus Bayern - Alien, Roboter oder Kristallwesen?",
		headline: "Alien, Roboter oder Kristallwesen? Was wurde in Bayern gesichtet?",
		img: "/images/blog/mysterioeses-ding-mischung-aus-alien-roboter-und-kristall.webp",
		alt: "Ein mysteriöses Ding, das wie eine Mischung aus einem Alien, einem Roboter und einem Kristall aussieht. Schwebend über dem Boden.",
		pubDate: "2023-08-09T00:00:00.000Z",
		url: "kristallwesen-sichtung-in-wunderlichhausen-bayern",
	},
	{
		title: "Gedankenlesendes Bakterium entdeckt - Teil 2",
		headline:
			"Unglaublich - Gedankenlesendes Bakterium auf dem Mars entdeckt - Teil 2",
		img: "/images/blog/gruenes-bakterium-blaue-fluessigkeit.webp",
		alt: "Ein grünes Bakterium in einer blauen Flüssigkeit. Im Hintergrund ist eine Wissenschaftlerin zu sehen.",
		pubDate: "2023-08-01T00:00:00.000Z",
		url: "gedankenlesendes-bakterium-auf-dem-mars-entdeckt-teil-2",
	},
	{
		title: "UFO oder fortschrittlicher Satellit? Was ist hier zu sehen?",
		headline: "UFO oder fortschrittlicher Satellit? Was ist hier zu sehen?",
		img: "/images/blog/zwei-wissenschaftler-untersuchen-teile-eines-ufos-auf-einem-schwarz-weiß-bild.webp",
		alt: "Zwei Wissenschaftler untersuchen Teile eines Ufos. Schwarz Weiß Foto aus dem Jahr 1970",
		pubDate: "2023-07-21T00:00:00.000Z",
		url: "foto-eines-ufos-in-einem-lagerhaus-1970",
	},
	{
		title: "UFO-Alarm in Kuhlingen",
		headline:
			"UFO-Alarm in Kuhlingen: Außerirdische entdecken ihre Leidenschaft für bayerische Kühe!",
		img: "/images/blog/ufos-mit-kuehen-auf-dem-feld-01.webp",
		alt: "UFOs mit Kühen auf einem Feld",
		pubDate: "2023-07-13T00:00:00.000Z",
		url: "ufo-alarm-in-kuhlingen-in-bayern",
	},
	{
		title: "Ein grünes Rind mit Rockstar-Ambitionen",
		headline:
			"Ein ungewöhnliches Rind mit Rockstar-Ambitionen: Das grüne Rind, das Gitarre spielt",
		img: "/images/blog/gruenes-rind-mit-gitarre.webp",
		alt: "Ein grünes Rind spielt auf der Weide auf einer Gitarre",
		pubDate: "2023-07-09T00:00:00.000Z",
		url: "gruenes-rind-spielt-auf-gitarre",
	},
	{
		title: "Geheimnisvolle Maschine: Mensch im Zentrum",
		headline: "Enthüllt: Die geheimnisvolle Blaupause",
		img: "/images/blog/geheimnisvolle-blaupause-mit-mensch-in-der-mitte.webp",
		alt: "Eine Blaupause einer großen mysteriösen Maschine mit einem Menschen in der Mitte",
		pubDate: "2023-07-02T00:00:00.000Z",
		url: "mysterioese-maschine-blaupause-mit-mensch",
	},
	{
		title: "Die NASA schickt ein riesiges Gehirn ins Weltall",
		headline: "Die NASA schickt ein riesiges Gehirn ins Weltall",
		img: "/images/blog/gehirn-auf-einer-rakete-im-weltall.webp",
		alt: "Ein großes Gehirn auf dem Rücken einer Rakte im Weltall",
		pubDate: "2023-06-26T00:00:00.000Z",
		url: "nasa-schickt-riesiges-gehirn-ins-weltall",
	},
	{
		title: "Großes Ameisenaufgebot vor dem Brandenburger Tor",
		headline:
			"Großes Ameisenaufgebot vor dem Brandenburger Tor: Die Insektenrevolution für den Klimaschutz",
		img: "/images/blog/ameisen-protestieren-in-berlin-vor-dem-brandenburger-tor.webp",
		alt: "Viele große Ameisen protestieren in Berlin vor dem Brandenburger Tor",
		pubDate: "2023-06-16T00:00:00.000Z",
		url: "ameisenrevolution-brandenburger-tor-klimaschutz",
	},
	{
		title: "Fliegende Zombies: Die neue Bedrohung aus der Luft",
		headline: "Fliegende Zombies: Die neue Bedrohung aus der Luft",
		img: "/images/blog/zombie-reitet-auf-hai.webp",
		alt: "Ein Zombie reitet auf einem Hai",
		pubDate: "2023-06-12T00:00:00.000Z",
		url: "fliegende-zombies-phaenomen",
	},
	{
		title: "Elektroautos besonders beliebt bei UFO-Entführungen",
		headline: "Unglaublich - Elektroautos beliebt bei Aliens",
		img: "/images/blog/elektroauto-wird-von-ufo-entfuehrt.webp",
		alt: "Ein rotes Elektroauto wird gerade von einem UFO entführt",
		pubDate: "2023-06-05T00:00:00.000Z",
		url: "elektroautos-beliebt-bei-ufo-entfuehrungen",
	},
	{
		title: "Valdar spricht: Interview mit einem Reptiloiden",
		headline:
			"Valdar der Reptiloid: Ein ungewöhnlicher Urlaub am legendären Strand von Kreta",
		img: "/images/blog/reptiloide-mit-strohhut.webp",
		alt: "Ein Reptiloide mit einem Strohut am Strand",
		pubDate: "2023-05-18T00:00:00.000Z",
		url: "interview-reptiloid-valdar-auf-kreta",
	},
	{
		title: "Gigantischer Gorilla erschüttert Frankfurt",
		headline: "Breaking News: Gigantischer Gorilla in Frankfurter Skyline gesichtet!",
		img: "/images/blog/gigantischer-gorilla-in-frankfurt.webp",
		alt: "Gigantischer Gorilla auf einem Dach eines Hochhauses in Frankfurt",
		pubDate: "2023-05-05T00:00:00.000Z",
		url: "gigantischer-gorilla-in-frankfurt-am-rhein-gesichtet",
	},
	{
		title: "Männliches Einhorn im magischen Wald entdeckt",
		headline:
			"Exklusiv - Männliches Einhorn in einem magischen Wald im Harz entdeckt",
		img: "/images/blog/goldenes-einhorn.webp",
		alt: "Ein goldenes Einhorn mit blauem Haar",
		pubDate: "2023-03-13T00:00:00.000Z",
		url: "maennliches-einhorn-im-harz-entdeckt",
	},
	{
		title: "Forscher entdecken antikes Sternentor in Deutschland",
		headline:
			"Sensationelle Entdeckung: Forscher finden antikes Sternentor in der Nähe der Stadt Speyer in Deutschland",
		img: "/images/blog/sternentor-in-deutschland-entdeckt.webp",
		alt: "Ein großes steinernes rundes Sternentor mit zwei Archäologen",
		pubDate: "2023-02-17T00:00:00.000Z",
		url: "antikes-sternentor-entdeckung-speyer",
	},
	{
		title: "Gitarre spielende Fische im Lake Tahoe entdeckt",
		headline: "Wow - Gitarre spielende Fische im Lake Tahoe entdeckt",
		img: "/images/blog/fische-spielen-gitarre.webp",
		alt: "Zwei Fische spielen Gitarre",
		pubDate: "2023-02-11T00:00:00.000Z",
		url: "gitarre-spielende-fische-lake-tahoe",
	},
	{
		title: "Mars-Entdeckung: Bakterium liest Gedanken",
		headline: "Unglaublich - Gedankenlesendes Bakterium auf dem Mars entdeckt",
		img: "/images/blog/gruenes-bakterium-blaue-fluessigkeit.webp",
		alt: "Ein grünes Bakterium in einer blauen Flüssigkeit. Im Hintergrund ist eine Wissenschaftlerin zu sehen.",
		pubDate: "2023-02-08T00:00:00.000Z",
		url: "gedankenlesendes-bakterium-auf-dem-mars-entdeckt",
	},
	{
		title: "Mars-Fusionsreaktor: Ein Sprung in die Zukunft",
		headline:
			"Durchbruch im Bereich Fusion - Marsianer und Terraner gelingt Durchbruch.",
		img: "/images/blog/fusionskraftwerk-auf-dem-mars.webp",
		alt: "Ein modernes Funsionskraftwerk auf dem Mars",
		pubDate: "2023-01-27T00:00:00.000Z",
		url: "fusionsreaktor-durchbruch-mars",
	},
	{
		title: "Silvester 2022: Weltuntergang verhindert durch Clowns",
		headline: "Silvester 2022: Weltuntergang verhindert durch Clowns",
		img: "/images/blog/clowns-retten-die-welt.webp",
		alt: "Zwei Clowns stehen um die Erdkugel herum",
		pubDate: "2023-01-01T00:00:00.000Z",
		url: "silversternacht-2022-weltuntergang-verhindert-durch-clowns",
	},
	{
		title: "Spektakuläre Entdeckung in Kuhlingen - UFO landet auf Koppel",
		headline: "Spektakuläre Entdeckung auf der Farm - UFO landet auf Koppel",
		img: "/images/blog/ufos-mit-kuehen-auf-dem-feld.webp",
		alt: "Eine Kuh schaut lustig in die Kamera. Im Hintergrund sind UFOs zu sehen.",
		pubDate: "2022-11-19T00:00:00.000Z",
		url: "ufo-landung-auf-koppel-in-kuhlingen",
	},
];
